...
var result = shell.exec(process.execPath+' -e \"console.log(1234);\"');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
assert.ok(result.stdout === '1234\n' || result.stdout === '1234\nundefined\n'); // 'undefined' for v0.4

// check if stderr goes to output
var result = shell.exec(process.execPath+' -e \"console.error(1234);\"');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
...
var result = shell.exec(process.execPath+' -e \"console.error(1234); console.log(666);\"');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
...
// check exit code
var result = shell.exec(process.execPath+' -e \"process.exit(12);\"');
assert.ok(shell.error());
assert.equal(result.code, 12);

// interaction with cd
shell.cd('resources/external');
var result = shell.exec(process.execPath+' node_script.js');
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
...
var result = shell.exec( util.format(process.execPath+' -e "console.log(%s);"', "\\\"\\'+\\'_\\'+\\'\\\"") );
assert.equal(shell.error(), null);
assert.equal(result.code, 0);
...
// set timeout option
result = shell.exec(process.execPath+' resources/exec/slow.js 100'); // default timeout is ok
assert.ok(!shell.error());
assert.equal(result.code, 0);
if (process.version >= 'v0.11') { // this option doesn't work on v0.10
  result = shell.exec(process.execPath+' resources/exec/slow.js 100', {timeout: 10}); // times out
  assert.ok(shell.error());
}
...
// no callback
var c = shell.exec(process.execPath+' -e \"console.log(1234)\"', {async:true});
assert.equal(shell.error(), null);
...
//
shell.exec(process.execPath+' -e \"console.log(5678);\"', function(code, stdout, stderr) {
  assert.equal(code, 0);
  ...
  //
  shell.exec(process.execPath+' -e \"console.log(5566);\"', {async:true}, function(code, stdout, stderr) {
    assert.equal(code, 0);
    ...
    //
    shell.exec(process.execPath+' -e \"console.log(5678);\"', {silent:true}, function(code, stdout, stderr) {
      assert.equal(code, 0);
      assert.ok(stdout === '5678\n' || stdout === '5678\nundefined\n');  // 'undefined' for v0.4
      assert.ok(stderr === '' || stderr === 'undefined\n');  // 'undefined' for v0.4
